import { collection, getDocs, addDoc, serverTimestamp, doc, setDoc, query, where, limit } from 'firebase/firestore';
...
export async function initializeSampleData() {
  try {
    ...
    // Check portfolio
    const portfolioSnap = await getDocs(query(collection(db, 'portfolio'), limit(1)));
    ...
    // Check posts (using published filter to avoid permission errors for non-admins)
    const postsSnap = await getDocs(query(collection(db, 'posts'), where('published', '==', true), limit(1)));
    if (postsSnap.empty) {
      const samples = [
        {
    ...
    console.warn("Sample data initialization skipped or failed:", error);
  }
}
